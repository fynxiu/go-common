...
func NewSnowflake(st Settings) (*Snowflake, error) {
	sf := new(Snowflake)
	sf.mutex = new(sync.Mutex)
	sf.sequence = uint16(1<<BitLenSequence - 1)

	if st.StartTime.After(time.Now()) {
		return nil, errors.New("new snowflake failed, StartTime of Settings should not after now")
	}
	...
	if err != nil {
		return nil, fmt.Errorf("new snowflake failed, %v", err)
	}
	if st.CheckMachineID != nil && !st.CheckMachineID(sf.machineID) {
		return nil, errors.New("new snowflake failed, machine id check failed")
	}
